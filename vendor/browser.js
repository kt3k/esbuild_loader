...
function validateInitializeOptions(options) {
  ...
  let wasmPath = getFlag(options, keys, "wasmPath", mustBeString);
...
var initialize = (options) => {
  ...
  if (!wasmPath)
    throw new Error('Must provide the "wasmPath" option');
  if (initializePromise)
    throw new Error('Cannot call "initialize" more than once');
  initializePromise = startRunningService(wasmPath, false);
  initializePromise.catch(() => {
    initializePromise = void 0;
  });
  return initializePromise;
};
var startRunningService = async (wasmPath, useWorker) => {
  let wasm = await Deno.readFile(wasmPath);
